...
# then log output of file /code/bin/cspltnd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/cspltnd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/cspltnd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/cspltnd /usr/bin/cspltnd
