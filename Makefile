...
HTTPS_GIT := github.com/cosmopolitan/cosmopolitan.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=cosmopolitan \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=cspltnd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/cspltnd ./cmd/cspltnd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/cspltnd.exe ./cmd/cspltnd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/cspltnd
...
draw-deps:
	...
	@goviz -i ./cmd/cspltnd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c cosmopolitan --local -f chains.yaml
endif
...
set-testnet-configs:
	cspltnd config set client chain-id chainid-1
	cspltnd config set client keyring-backend test
	cspltnd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | cspltnd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | cspltnd keys add acc1 --recover`
